...
export function FieldView({ field, pathPrefix, filled }: { field: TemplateField; pathPrefix: string; filled?: FilledValues }) {
  ...
  return (
    <div className="border-l-2 border-neutral-200 hover:border-petrol/40 pl-4 py-2 transition-colors">
          ...
          {field.options.map((opt) => {
            ...
            return (
              <div key={opt.key}>
                ...
                {optTextQuote && (
                  <p className="ml-6 text-xs text-neutral-400 italic">from transcript: "{optTextQuote}"</p>
                )}
              </div>
            );
          })}
      ...
      {quote && (
        <p className="mt-1 text-xs text-neutral-400 italic">from transcript: "{quote}"</p>
      )}

      {field.children && field.children.length > 0 && (
        <div className="mt-2 space-y-1">
          {field.children.map((child) => (
            <FieldView key={child.key} field={child} pathPrefix={path} filled={filled} />
          ))}
        </div>
      )}
    </div>
