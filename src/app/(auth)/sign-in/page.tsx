...
export default async function SignInPage({
  ...
  const error = params?.error ? t(STRINGS.signInErrorInvalid, locale) : undefined;
      ...
      <section className="hidden md:flex flex-col justify-between bg-petrol-deep text-white p-10">
        ...
        <div>
          <h1 className="text-2xl font-semibold leading-snug">
            {t(STRINGS.signInTagline, locale)}
          </h1>
          <p className="text-white/70 text-sm mt-3 max-w-sm">{t(STRINGS.signInSubtext, locale)}</p>
        </div>
        ...
        <form action={signInAction} className="w-full max-w-sm space-y-4">
          <div>
            <p className="text-xs font-bold tracking-widest uppercase text-petrol">
              {t(STRINGS.welcomeBack, locale)}
            </p>
            <h2 className="text-xl font-semibold mt-1">{t(STRINGS.signInHeading, locale)}</h2>
          </div>
          {error && (
            <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2">{error}</p>
          )}
          <label className="block text-sm font-semibold">
            {t(STRINGS.workEmailLabel, locale)}
            <input
              name="email"
              type="email"
              required
          ...
          <label className="block text-sm font-semibold">
            {t(STRINGS.passwordLabel, locale)}
            <input
              name="password"
              type="password"
              required
